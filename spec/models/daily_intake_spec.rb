...
RSpec.describe DailyIntake, type: :model do
  ...
  let(:food_to_add) { { foods: food.id } }

  describe '#add_foods' do
    it 'adds food in the daily intake' do
      expect { daily_intake.add_foods(food_to_add) }.to change { daily_intake.foods.count }.by(1)
    end
  end
  ...
  describe '#add_food_from_meal' do
    it 'adds meal in the daily intake' do
      ...
      expect { daily_intake.add_foods_from_meals([meal]) }.to change { daily_intake.foods.count }.by(meal.foods.count)
    end
  end

    describe '#total_calories' do
    it 'returns the total calories of the daily intake' do
      daily_intake.add_foods(food_to_add)
